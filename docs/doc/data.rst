...
You will often see a “_score” field in the returned variant object, which is the internal score representing how well the query matches the returned variant object. It probably does not mean much in `variant annotation service <data.html>`_ when only one variant object is returned. In `variant query service <variant_query_service.html>`_, by default, the returned variant hits are sorted by the scores in descending order.

_version field
--------------

Sometime, you will see a “`_version`” field in the returned variant object, e.g. from the `v1/variant <variant_annotation_service.html>`_ endpoint. This field is basically for our internal information purpose, not very useful to the end users. You can just ignore it.
